...
const ACCURATE_HINT = '精确模型更大，首次加载会明显更久，但更适合长公式、矩阵和多行公式。';
...
let fastReady = false, accurateReady = false, recognizing = false, pendingFast = false;
...
function stripDelimiters(s) {
  let t = String(s || '').trim();
  for (const [a,b] of [['\\[','\\]'], ['\\(','\\)'], ['$$','$$']]) if (t.startsWith(a) && t.endsWith(b)) t = t.slice(a.length, -b.length).trim();
...
function looksRunaway(text) {
  ...
  return sameTokenRun || repetitive || bangCount > 24 || t.length > 1400;
}
...
async function loadImage(file, auto = true) {
  ...
  if (auto) {
    if (fastReady) recognizeFast();
    else { pendingFast = true; setStatus(`图片已就绪，快速模型加载完成后会自动识别。${FAST_HINT}`); }
  }
}
...
function clearAll() {
  ...
  setStatus(fastReady ? '快速模型已就绪。粘贴截图或上传公式图片即可识别。' : `正在加载快速模型。${FAST_HINT}`, fastReady ? 'ok' : '');
}
...
async function preprocessImage(file) {
  ...
  const d=octx.getImageData(0,0,target,target).data,pixels=new Float32Array(target*target),mean=.7931,std=.1738;for(let i=0,p=0;i<d.length;i+=4,p++)pixels[p]=(d[i]/255-mean)/std;return pixels;
}
...
async function recognizeFast() {
  ...
  recognizing = true; lastEngine='fast'; recognizeBtn.disabled=true; accurateBtn.disabled=true; setProgress(100); setStatus('正在用快速模型识别…');
  try { const pixels=await preprocessImage(currentFile); worker.postMessage({type:'predict-fast',pixels:pixels.buffer},[pixels.buffer]); }
...
function recognizeAccurate(autoFallback=false) {
  if (!currentFile || recognizing) return;
  recognizing=true; lastEngine='accurate'; recognizeBtn.disabled=true; accurateBtn.disabled=true; setProgress(0);
  setStatus((autoFallback ? '检测到快速模型输出异常，正在自动切换精确模型。' : '正在加载/使用精确模型识别。') + ACCURATE_HINT, 'warn');
...
worker.addEventListener('message', (event) => {
  ...
  if(data.type==='progress'){
    ...
    setStatus(`${engine==='accurate'?'精确':'快速'}模型正在加载${file}${Number.isFinite(p)?`（${Math.round(p)}%）`:''}。${engine==='accurate'?ACCURATE_HINT:FAST_HINT}`);
  } else if(data.type==='ready'){
    if(engine==='fast'){fastReady=true;recognizeBtn.disabled=!currentFile;setProgress(100);setStatus('快速模型已就绪。复杂公式可点“精确识别”。','ok');if(pendingFast&&currentFile){pendingFast=false;recognizeFast();}}
    else {accurateReady=true;setStatus('精确模型已加载，正在识别复杂公式…');}
  } else if(data.type==='result'){
    recognizing=false;const text=stripDelimiters(data.text||'');
    ...
    const label=engine==='accurate'?'精确识别':'快速识别';
    setStatus(text?`${label}完成。可复制到 Word / MathType，也可以先修改 LaTeX。`:`${label}没有得到有效公式，建议裁剪紧一些后再试。`,text?'ok':'warn');showToast(text?`${label}完成`:'未识别到公式');
  } else if(data.type==='error'){
    recognizing=false;recognizeBtn.disabled=!currentFile||!fastReady;accurateBtn.disabled=!currentFile;
    setStatus(`${engine==='accurate'?'精确':'快速'}模型发生错误：${data.message||'未知错误'}。`, 'error');
  }
});
...
fileInput.addEventListener('change',()=>loadImage(fileInput.files?.[0]));clearBtn.addEventListener('click',clearAll);recognizeBtn.addEventListener('click',recognizeFast);accurateBtn.addEventListener('click',()=>recognizeAccurate(false));latexEl.addEventListener('input',renderFormula);

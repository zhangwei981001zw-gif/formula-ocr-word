...
const ACCURATE_MODEL_ID = 'onnx-community/TexTeller-ONNX';
...
async function predictFast(buffer) {
  ...
  const outputs = await fastModel.generate({ inputs: pixelValues });
...
self.addEventListener('message', async (event) => {
  ...
  try {
    if (data.type === 'init-fast') {
      await initFast();
    } else if (data.type === 'init-accurate') {
      await initAccurate();
    } else if (data.type === 'predict-fast') {
      await predictFast(data.pixels);
    } else if (data.type === 'predict-accurate') {
      await predictAccurate(data.image);
    }
  } catch (err) {
    // init/predict functions have already emitted detailed errors when possible.
